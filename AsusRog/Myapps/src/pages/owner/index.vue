<template>
<div class="q-pa-md">
    <h3 style="align: center">BERIKUT MERUPAKAN NAMA OWNER </h3>
    <div class="row">
        <div class="col" style="align : center">
            <!-- Qlist awal -->
            <q-list bordered class="rounded-borders q-mx-auto bg-secondary" style="max-width: 1100px">
                <q-item-label header>
                    <q-btn color="red" icon="add_box" label="OWNER" size="sm" @click="add()" />
                    <q-btn color="blue" icon="add_box" label="Contoh" size="sm" @click="contoh()"/>
                </q-item-label>
                ...
                <!-- Tabel -->
                <q-item v-for="(owner, index) in owners" :key="owner.id" class="bg-grey-3" line="1">
                    ...
                    <q-item-section top class="col-2 gt-xm flex flex-center">
                        <q-item-label class="q-mt-sm">{{owner.Nama}}</q-item-label>
                    </q-item-section>

                    <q-item-section top class="col-2 gt-xm flex flex-center">
                        <q-item-label class="q-mt-sm">{{owner.JenisKelamin}}</q-item-label>
                    </q-item-section>

                    <q-item-section top class="col-2 gt-xm flex flex-center">
                        <q-item-label class="q-mt-sm">{{owner.TanggalLahir}}</q-item-label>
                    </q-item-section>

                    <q-item-section top class="col-2 gt-xm flex flex-center">
                        <q-item-label class="q-mt-sm">{{owner.NoTelepon}}</q-item-label>
                    </q-item-section>

                    <q-item-section top class="col-2 gt-xm flex flex-center">
                        <q-item-label class="q-mt-sm">{{owner.Email}}</q-item-label>
                    </q-item-section>

                    <q-item-section top class="col-1 gt-xm flex flex-center">
                        <q-item-label lines="1">

                            <q-btn size="12px" flat dense round icon="edit" @click="edit(owner)" />
                            <q-btn size="12px" flat dense round icon="delete" @click="onDelete(owner.id)" />
...
<script>
import owner from '../../api/owner/index';
    ...
    methods: {
        ...
        onDelete(id) {
            if (confirm('Apakah anda yakin akan menghapus data ini ?')) {
                owner.deleteowner(window, id)
                    .then((res) => {
                        owner.getowner(window)
                            .then((res) => {
                                this.owners = res.data
                                this.$router.go('/owner')
                            })
                            .catch(() => {
        ...
        onSubmit() {
            const self = this
            owner.postowner(window, self.form.Nama, self.form.JenisKelamin,
                                self.form.TanggalLahir, self.form.NoTelepon,
                                self.form.Email)
                .then(function (result) {
                    self.$router.go('/owner')
                })
                .catch(function (err) {
                    console.log(err);
                });
        ...
        edit(owner) {
            if (confirm('Anda akan di alihkan ke halaman Edit data, Tekan OK untuk lanjut atau Batal untuk kembali')) {
                try {
                    ...
                    this.form.Nama = owner.Nama
                    this.form.JenisKelamin = owner.JenisKelamin
                    this.form.TanggalLahir = owner.TanggalLahir
                    this.form.NoTelepon = owner.NoTelepon
        ...
        update(id) {
            const self = this
            owner.putowner(window, self.form.id, self.form.Nama, 
